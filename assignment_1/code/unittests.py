...
from modules import CrossEntropyModule, LinearModule, ReLUModule, SoftMaxModule
...
class TestBatchNorm(unittest.TestCase):
    ...
    def test_autograd(self):
        ...
        for test_num in range(10):
            ...
            y_auto = bn_auto(x)
            self.assertLess(
                np.max(np.abs(y_auto.mean(dim=0).data.numpy())), 1e-5)
            self.assertLess(
                np.max(np.abs(y_auto.var(dim=0).data.numpy() - 1)), 1e-1)
    ...
    def test_manual_function(self):
        ...
        for test_num in range(5):
            ...
            x = 2 * torch.randn(n_batch, n_neurons, requires_grad=True) + 10
            input = x.double()
            gamma = torch.sqrt(10 * torch.arange(n_neurons,
                                                 dtype=torch.float64, requires_grad=True))
            beta = 100 * torch.arange(n_neurons,
                                      dtype=torch.float64, requires_grad=True)
            ...
            self.assertLess(np.max(np.abs(y_manual_fct.mean(
                dim=0).data.numpy()) - np.arange(0, n_neurons * 100, 100)), 1e-5)
            self.assertLess(np.max(np.abs(y_manual_fct.var(
                dim=0).data.numpy() - np.arange(0, n_neurons * 10, 10))), 1)
    ...
    def test_manual_module(self):
        ...
        for test_num in range(10):
            ...
            self.assertLess(
                np.max(np.abs(y_manual_mod.mean(dim=0).data.numpy())), 1e-5)
            self.assertLess(
                np.max(np.abs(y_manual_mod.var(dim=0).data.numpy() - 1)), 1e-1)
...
class TestLosses(unittest.TestCase):
    ...
    def test_crossentropy_loss(self):
        ...
        for test_num in range(10):
            ...
            def f(_): return CrossEntropyModule().forward(X, y)
